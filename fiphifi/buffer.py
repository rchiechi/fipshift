...
class Buffer(threading.Thread):
        ...
        try:
            ...
            with open(_ts, 'rb') as fh:
                ...
                if self.icecast.send_data(data):
                    # Mark file for deletion after successful send
    ...
    def cleanup_sent_files(self):
        ...
        for ts_file in self.sent_files:
            try:
                # Keep files for a short period as safety buffer
                if current_time - os.path.getmtime(ts_file) > self.duration * 2:
                    os.remove(ts_file)
                    files_to_remove.add(ts_file)
                    logger.debug(f"Removed sent file: {ts_file}")
            except (OSError, FileNotFoundError) as e:
                logger.warning(f"Error removing file {ts_file}: {e}")
                files_to_remove.add(ts_file)
        ...
        for ts_file in self.sent_files:
            try:
                if os.path.exists(ts_file):
                    os.remove(ts_file)
                    logger.debug(f"Cleaned up file: {ts_file}")
            except OSError as e:
